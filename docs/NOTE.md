...
| 3 | One owner for "run this in the editor page" | Worth exploring | `RESOLVE_BODY` is orphaned in code.js while its siblings live in commands.js; the string-body seam compiles against a `PRELUDE` the bodies cannot see; the harness runs commands *synchronously*, so the real timeout/clobber/unparsable behaviour is untested. (First step landed 2026-09-25 with candidate 1: `RESOLVE_BODY` now lives in `commands.js` next to its siblings.) |
